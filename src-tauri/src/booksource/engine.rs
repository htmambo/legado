...
async fn run_source_fn(
    ...
    let bootstrap = build_call_js(&req_id, file_name, &source_code, fn_candidates, &fn_args)?;
    window
        .eval(bootstrap)
...
fn build_call_js(
    ...
    let candidates =
        serde_json::to_string(fn_candidates).map_err(|e| CommandError::other(e.to_string()))?;
    let args = serde_json::to_string(fn_args).map_err(|e| CommandError::other(e.to_string()))?;

    // 模块工厂：new Function('legado', 源码 + return 导出表)
    // —— IIFE 隔离全局；candidates 取第一个存在的函数。
    Ok(format!(
